...
We used the abmAnimalMovement v.0.1.3.0 (\protect\hyperlink{ref-abmAnimalMovement}{Marshall \& Duthie, 2022}) package to simulate the movement data of an animal with a predefined (i.e., known) habitat preference.
...
We used that simulated landscape and abmAnimalMovement to simulate a population of 20, that was later sampled from.
...
The area-based Compana approach seemed more insensitive to sample size than the other methods (\(\beta\) 0.07; 95\% HDCI -0.51 - 0.65), instead benefiting from increases in tracking duration (\(\beta\) -0.11; 95\% HDCI -0.14 - -0.07) and frequency (\(\beta\) -0.12; 95\% HDCI -0.16 - -0.09; Fig. \ref{fig:effectPlotArea}).
This contrasts strongly with the Poisson model approach, where all increases in sampling intensity lead to a reduction in deviation from the median estimate: sample size (\(\beta\) -0.29; 95\% HDCI -0.46 - -0.13), tracking duration (\(\beta\) -0.39; 95\% HDCI -0.39 - -0.38), and tracking frequency (\(\beta\) -0.47; 95\% HDCI -0.47 - -0.46; Fig. \ref{fig:effectPlotPois}).
The other two methods fall in-between with the sampling having a smaller impact.
The Step Selection model approach still benefited from increasing sampling duration (\(\beta\) -0.08; 95\% HDCI -0.08 - -0.07) and frequency, but the impact on increasing sample size was not apparent (\(\beta\) -0.08; 95\% HDCI -0.08 - -0.07; Fig. \ref{fig:effectPlotSSF}).

The points per step decision holds the largest improvement in terms of deviation from the median; the Poisson model approach was aided greatly by increases to the number of random points per step (\(beta\) -0.60; 95\% HDCI -0.60 - -0.59).
...
Unlike the points per step decision the naive averaging of the step selection models leading to a smaller spread of estimates is visible in the specification curve and confirmed by model beta estimate (\(beta\) -0.14; 95\% HDCI -0.14 - -0.13).
...
Here we see the benefits of including the step and turn angles in the model formula for the summarised SSF approach (\(beta\) 0.04; 95\% HDCI 0.03 - 0.05), but a contrasting effect for both the Two-step (\(beta\) 0.23; 95\% HDCI 0.21 - 0.26) and Poisson approaches (\(beta\) -0.02; 95\% HDCI -0.03 - -0.01) where the not integrated formula tends to reduce the spread of estimates (Fig. \ref{fig:effectPlotSSF}.
...
In brief, the use of MCPs (\(beta\) -0.23; 95\% HDCI -0.30 - -0.16), larger contours (areas) (\(beta\) -0.05; 95\% HDCI -0.09 - -0.01), and type III designs (\(beta\) -0.18; 95\% HDCI -0.25 - -0.11) tended to lead to more variable results.
...
We used \emph{R} v.4.3.3 (\protect\hyperlink{ref-base}{R Core Team, 2023}) via \emph{RStudio} v.2023.09.1 (\protect\hyperlink{ref-rstudio}{RStudio Team, 2022}).
