...
For disciplines where direct replications are more common and feasible, replication efforts can play a feedback <!-- AKA diagonistic role. Peterson pano 2021 --> role in science [@open_science_collaboration_estimating_2015; @freedman_economics_2015].
...
For the best model we followed recommendations of @silva_autocorrelationinformed_2022, opting to run AKDEs as weighted and using (PREML???) as both these options tend to be the most robust in scenarios of lower tracking frequencies.
...
The area based method – Compositional Analysis (Compana) – returned a median estimate that agreed with the underlying known simulation preference (i.e., preference for habitat 2; Fig. \@ref(fig:specCurveArea)).
Despite this overall agreement, the final estimates ranged all the way down to minimal preference that are failing to detect the simulated preference.
When broken down by decision, all decisions bar higher sample sizes, appear to have a large range in final estimates.
The sample size decisions has a clear reduction in possible answers when the sample is 10 or 20 individuals.
...
The specification curve generated from summarised Step Selection Functions (SSFs), similarly reveals a median estimate has agrees with the simulated parametrisation (Fig. \@ref(fig:specCurveSSF)).
Unlike the Compana curve, the SSF curve has far fewer results that indicate very low habitat preference.
However, the SSF method did results in estimates indicating avoidance of habitat 2 that contradicts the underlying simulated process; these negative preference results are in a small minority.
The clearest structure in the estimates resulting from different decisions belong to changes in sample size and tracking regime.
...
Model formulation did have an impact, with the integrated formulation was more consistent at retrieving positive preference, but the spread was much larger.
...
<!-- The conditional *R^2^* values differed for the three models.  -->
...
<!-- The marginal *R^2^* represents the bulk of the conditional *R^2^* suggesting an important role for the fixed/population effects. -->
<!-- The Compana results model had a conditional *R^2^* of `r round(r2Outputs$R2[r2Outputs$model == "areaBrms" & r2Outputs$Component == "marginal"], digits = 2)`; whereas the SSF model returned `r round(r2Outputs$R2[r2Outputs$model == "ssfBrms" & r2Outputs$Component == "marginal"], digits = 2)`; the Two-Step model returned `r round(r2Outputs$R2[r2Outputs$model == "twoStepBrms" & r2Outputs$Component == "marginal"], digits = 2)`; and the Poisson model returned `r round(r2Outputs$R2[r2Outputs$model == "poisBrms" & r2Outputs$Component == "marginal"], digits = 2)` -->
...
The area-based Compana approach seemed more insensitive to sample size than the other methods ($\beta$ `r beta_areaBrms_b_sampleSizeScaled`), instead benefiting from increases in tracking duration ($\beta$ `r beta_areaBrms_b_trackDuraScaled`) and frequency ($\beta$ `r beta_areaBrms_b_trackFreqScaled`; Fig. \@ref(fig:effectPlotArea)).
This contrasts strongly with the Poisson model approach, where all increases in sampling intensity lead to a reduction in deviation from the median estimate: sample size ($\beta$ `r beta_poisBrms_b_sampleSizeScaled`), tracking duration ($\beta$ `r beta_poisBrms_b_trackDuraScaled`), and tracking frequency ($\beta$ `r beta_poisBrms_b_trackFreqScaled`; Fig. \@ref(fig:effectPlotPois)). 
The other two methods fall in-between with the sampling having a smaller impact.
The Step Selection model approach still benefited from increasing sampling duration ($\beta$ `r beta_ssfBrms_b_trackFreqScaled`) and frequency, but the impact on increasing sample size was not apparent ($\beta$ `r beta_ssfBrms_b_trackFreqScaled`; Fig. \@ref(fig:effectPlotSSF)).
...
The points per step decision holds the largest improvement in terms of deviation from the median; the Poisson model approach was aided greatly by increases to the number of random points per step ($beta$ `r beta_poisBrms_b_availablePerStepScaled`).
...
Unlike the points per step decision the naive averaging of the step selection models leading to a smaller spread of estimates is visible in the specification curve and confirmed by model beta estimate ($beta$ `r beta_ssfBrms_b_averagingMethodNaiveaverage`).
...
Here we see the benefits of including the step and turn angles in the model formula for the summarised SSF approach ($beta$ `r beta_ssfBrms_b_averagingMethodNaiveaverage`), but a contrasting effect for both the Two-step ($beta$ `r beta_twoStepBrms_b_modelFormulamf.ss`) and Poisson approaches ($beta$ `r beta_poisBrms_b_averagingMethodNaiveaverage`) where the not integrated formula tends to reduce the spread of estimates (Fig. \@ref(fig:effectPlotSSF).
...
In brief, the use of MCPs ($beta$ `r beta_areaBrms_b_areaMethodMCP`), larger contours (areas) ($beta$ `r beta_areaBrms_b_contourScaled`), and type III designs ($beta$ `r beta_areaBrms_b_typeIII`) tended to lead to more variable results.
...
```{r allEffectsPlot, echo=FALSE, out.width='100%', fig.height=5, fig.width=3, fig.align="centre", fig.cap="Beta coefs"}
...
Fortunately the vast majority of the answers from the <!-- add number here --> analysis end points agree, and correctly recover the positive habitat selection programmed into the simulated animals.
...
<!-- different methods benefit differently to different decisions, ie no one size fits all -->
<!-- overall spec curves suggest poisson and ssf are the most stable with the fewer instances of null answers -->

<!-- highlights the possibilities of trade off dura - freq - sample size. e.g., SSF is better to focus on tracking regime, poisson sees gains from all of them -->
<!-- highligths the need to watch out for some methods - two-step - as they appear much more sensitive to analysis choices than other methods -->
...
Despite this the frequency of MCP use in spatial ecology [@crane_lots_2021], we felt, warranted its inclusion as a conceivable choice.
...
The logistics of organising <!--245(?)--> researchers to assess uncertainty in analyses is infeasible on a broader scale.
...
Previous work with SSF has already highlighted the importance of more random locations per step for identifying use of smaller/rarer habitat types [@thurfjell_applications_2014].
...
<!-- Only be careful as the one contradicting the others might be right even in the minority -->
...
We used *R* `r paste0("v.", version$major, ".", version$minor)` [@base] via *RStudio* v.2023.09.1 [@rstudio]. <!-- version has to be manually added because rstudioapi doesn't work in targets -->
